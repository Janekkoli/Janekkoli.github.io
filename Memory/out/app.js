...
class DivinGame {
    ...
    onClick(event) {
        if (this.game.showPermit(this) && this.showing == false) {
...
class MemoryGame {
    ...
    cardsLeft = 10;
    numberOfTrys = 0;
    apiPoint = new ApiClass('http://127.0.0.1:8000/');
    currentlyShowing = new Set();
    constructor(divWithGame) {
        if (divWithGame == null)
            throw Error;
    ...
    showPermit(color) {
        if (this.currentlyShowing.size < 2) {
            this.numberOfTrys++;
            this.currentlyShowing.add(color);
            return true;
        }
    ...
    async newGame() {
        ...
        buttonElement.addEventListener('click', () => {
            ...
            this.cardsPutter(numberOfCards);
        });
    }
    async cardsPutter(numberOfCards) {
        let colors = [];
        try {
...
const divWithGame = document.getElementById("MemmoryGame");
